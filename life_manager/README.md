...
2. Dateien ersetzen.
3. Add-on neu bauen.
4. `/health` muss Version `1.5.2` liefern.
...
- Der Quest Manager zeigt unten `Frontend v1.5.2`, damit Cache-Probleme sofort erkennbar sind.


## v1.5.2 Achievements, Boss Fights & Streaks
...
The planner is intentionally rule-based in v1.5.2 so every recommendation remains
explainable. An optional AI layer can be added later without replacing the
deterministic core.


## v1.5.2 Planner Today Fix
...
## v1.5.2 Stable Home Assistant data transport
...
From v1.5.2 onward `/dashboard` also exposes the complete response under:
...
`examples/life_manager_sensor_v1.5.2.yaml`
...
v1.5.2 uses a permanent loader instead.
...
`/local/life-manager-1.5.2.js?v=1.5.2`
...
## v1.5.2 Scheduling UI + Analytics
...
## v1.5.2 Generated Home Assistant Bridge
...
first update to v1.5.2, perform one Home Assistant Core restart if the generated
...
## v1.5.2 Home Assistant package path detection
...
## v1.5.2 Home Assistant bridge fix
...
## v1.5.2 Standalone Integration Architecture

This release starts removing the Home Assistant package bridge.

### Architecture

Large Life Manager datasets now have a dedicated frontend endpoint:

`GET /frontend/dashboard`

They no longer need to be persisted as one giant Home Assistant state attribute.

A short-lived frontend session mechanism is also available so browser actions
can use temporary credentials rather than embedding the permanent API key.

### Compatibility

Existing Home Assistant sensors and scripts continue to work during migration.
Frontend cards retain sensor/script fallback behavior.

The add-on no longer writes `life_manager_generated.yaml`.

This is a migration release: the next frontend iterations can move individual
cards from Home Assistant state data to direct API reads without requiring new
Home Assistant YAML.


## v1.5.2 Startup hotfix

- Fixes the Python syntax error introduced by the v1.5.0 frontend-session insertion.
- Frontend session helpers now live at safe module scope.
- Fixes `/frontend/dashboard` to use `fetch_streaks_v2()` and
  `evaluate_achievements()`.
- Adds a mandatory backend compile gate to the repository validation.


## v1.5.2 Direct Today actions

The Today card no longer requires `script.life_quest_occurrence`.

At add-on startup a random action token is generated and added to the frontend
manifest together with `frontend_api_url`. The token rotates on every add-on
restart and is only accepted by the scoped frontend write endpoints.

New add-on option:

`frontend_api_url`

Default: `http://homeassistant.local:8000`

Set it once in the add-on configuration if your Life Manager API is reachable
under a different URL.

Direct actions now include complete, overcome, move to tomorrow, move to date,
and skip.
