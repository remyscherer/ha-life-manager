...
2. Dateien ersetzen.
3. Add-on neu bauen.
4. `/health` muss Version `1.8.0` liefern.
...
- Der Quest Manager zeigt unten `Frontend v1.8.0`, damit Cache-Probleme sofort erkennbar sind.


## v1.8.0 Achievements, Boss Fights & Streaks
...
The planner is intentionally rule-based in v1.8.0 so every recommendation remains
explainable. An optional AI layer can be added later without replacing the
deterministic core.


## v1.8.0 Planner Today Fix
...
## v1.8.0 Stable Home Assistant data transport
...
From v1.8.0 onward `/dashboard` also exposes the complete response under:
...
`examples/life_manager_sensor_v1.8.0.yaml`
...
v1.8.0 uses a permanent loader instead.
...
`/local/life-manager-1.8.0.js?v=1.8.0`
...
## v1.8.0 Scheduling UI + Analytics
...
## v1.8.0 Generated Home Assistant Bridge
...
first update to v1.8.0, perform one Home Assistant Core restart if the generated
...
## v1.8.0 Home Assistant package path detection
...
## v1.8.0 Home Assistant bridge fix
...
## v1.8.0 Home Assistant bridge restored
...
## v1.8.0 Home Assistant bridge fields hotfix
...
## v1.8.0 Occurrence diagnostic split
...
## v1.8.0 Occurrence entity rename
...
## v1.8.0 Complete Dashboard Card
...
## v1.8.0 Dashboard Management hotfix
...
## v1.8.0 Quest Editor focus hotfix
...
## v1.8.0 SQLite date/time compatibility
...
## v1.8.0 Quest bridge compatibility hotfix

The dashboard no longer uses the legacy manually-maintained
`script.life_quest_create`, `script.life_quest_update` and
`script.life_quest_toggle` scripts.

New generated bridge scripts:
- `script.life_manager_quest_create`
- `script.life_manager_quest_update`
- `script.life_manager_quest_toggle`

Weekdays are passed as a native list and are never processed with `.split()`.

The SQLite occurrence write path also normalizes occurrence/move dates to ISO
strings before storing them and logs a precise backend error if a write fails.


## v1.8.0 Home Assistant payload normalization
...
- `None`, `null` and empty strings
- numeric strings
- ISO date strings
- weekday lists
- stringified weekday lists such as `[1, 3, 5]`
- legacy comma-separated weekdays
...
Features:
- manual backups
- configurable manual-backup retention
- backup integrity checks
- list backups in the all-in-one dashboard
- restore backups from the Management tab
- automatic pre-restore safety backup
- delete old backups
- SQLite online backup API, safe with WAL mode
